// 모바일 홈(모바일 전용). 인사말 헤더 + 검색·필터 + 카테고리 칩 + 프로모 배너 + Featured 리스트.
...
// 라우트별 카테고리 아이콘(이모지 금지 → 인라인 SVG). 미매칭은 기본 태그 아이콘.
...
export async function MobileHome({ featured }: { featured: ProductListItem[] }) {
  ...
  return (
    <div className="flex flex-col gap-5 px-5 pt-4 md:hidden">
      {/* 인사말 헤더 */}
      ...
      {/* 검색 + 필터 */}
      <form action="/commercial" className="flex items-center gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-xl bg-neutral-100 py-3 pl-10 pr-3 text-sm text-neutral-700 placeholder:text-neutral-400"
          />
        </div>
        <Link
          href="/commercial"
      ...
      <div className="-mx-5 flex gap-4 overflow-x-auto px-5 pb-1">
        {menu.map((m) => (
          <Link key={m.id} href={m.route} className="flex shrink-0 flex-col items-center gap-1.5">
      ...
      <Link
        href={postHref}
        className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 p-5 text-white"
      >
        ...
        <svg
          viewBox="0 0 24 24"
          className="absolute -bottom-2 right-2 h-28 w-28 text-white/25"
          fill="currentColor"
        >
        ...
        <div className="-mx-5 flex gap-4 overflow-x-auto px-5 pb-2">
          {featured.map((p) => (
            <Link
              key={p.id}
              href={`/products/${p.id}`}
              className="w-60 shrink-0 overflow-hidden rounded-2xl border border-neutral-200 bg-white"
            >
